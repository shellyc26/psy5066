...
# [default, rladies, rladies-fonts, "my-theme.css"]
library(tidyverse)
library(knitr)
library(kableExtra)
```

![](images/lovehateprogramming.png)
...
We are going to use the [Avengers Dataset](https://shellyc26.github.io/psy5066/homework/homework-avengers.csv) that we used in homework 2. The question we want to answer is: 

--

* Is the **WorldwideGross** of Marvel Phase I, II, and III movies the same? 
  * Marvel Cinematic Universe Phase I: 2008 - 2012 
  * Marvel Cinematic Universe Phase II: 2013 - 2015 
  * Marvel Cinematic Universe Phase III: 2016 - 2019 
...
```{r, echo = FALSE}
avengers <- read.csv('https://shellyc26.github.io/psy5066/homework/homework-avengers.csv')

practice <- avengers %>% mutate(phase = ifelse(YearOfRelease %in% 2008:2012, 1, 
                                                       ifelse(YearOfRelease %in% 2013:2015, 2, 3))) %>%
  select(c(Movie, YearOfRelease, WorldwideGross, phase))
```

```{r}
summary(aov(WorldwideGross ~ phase, data = practice))
```
...
```{r}
avengers_w_phase <- avengers %>% mutate(phase = ifelse(YearOfRelease %in% 2008:2012, 1, 
                                                       ifelse(YearOfRelease %in% 2013:2015, 2, 3))) %>%
  select(c(Movie, YearOfRelease, WorldwideGross, phase)) %>%
...
```{r}
summary(aov(WorldwideGross ~ phase, data = avengers_w_phase))
```
]
--
...
So, the difference in **WorldwideGross** of Marvel Cinematic Universe Phase I, II, III is not statistically significant (*p*>.05).
...
- Multiple comparisons & contrasts (Tuesday)
- Twoway ANOVA (Thursday)
- Interactions with ANOVA (Tuesday)
- THANKSGIVING BREAK!!!