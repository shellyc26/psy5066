...
- You want independence of observations *within* your samples and *between* your samples for this analysis
...
We assume **homogeneity of variance** for independent samples *t*-tests

  - Even if samples come from different populations, these populations differ in mean but *not* variance
  - Is this practical?

--

Standard error is easy to calculate when you only know one *SD* (e.g., one-sample *t*-tests). But we have two *SD*s, so unless they're the exact same, we need to address this somehow. Maybe a *weighted combination* of the two. 

---

## Pop Quiz

How to do we calculate standard error of the mean?

How do we calculate a one-sample $t$-test?

Let's compare it to our independent samples $t$-test...how do we calculate that?
...
2. __Welch's *t*-test__ if the variances are NOT (roughly) the same. Slightly less powerful. You don't pool the variances int he same way
---
...
Final result is *SE of the difference of independent means*. 

- Remember that standard deviations estimate population variability (i.e., variability of population scores)

- Taking the next step and dividing the *SD* by the square root of some variation of $N$ (depending on the test equation) helps us estimate *variability of means* rather than scores. 
---
**Student's**
$$\large{\hat{\sigma}_D = \sqrt{\frac{(N_1-1)\hat{\sigma}^2_1 + (N_2-1)\hat{\sigma}^2_2}{N_1+N_2-2}} \sqrt{\frac{1}{N_1} + \frac{1}{N_2}}}$$
Note that in Student's calculation, the variance for the larger sample will be weighted more heavily because each individual variance is being multiplied by N-1. 

--

**Welch's **
$$\large{\hat{\sigma}_D = \sqrt{\frac{\hat{\sigma}^2_1}{N_1}+\frac{\hat{\sigma}^2_2}{N_2}}}$$
Meanwhile, in Welch's, the standard deviations are being *divided* by sample size, so smaller samples will be weighted more heavily. 
---
$$\large{\hat{\sigma}_D = \sqrt{\frac{\hat{\sigma}^2_1}{N_1}+\frac{\hat{\sigma}^2_2}{N_2}}}$$
```{r}
...
$$ t_{df} = \frac{\bar{X}_1-\bar{X}_2}{\sigma_D}$$ and then can find the probability of the absolute value of this test statistic (for two-tailed) or more extreme given the null is true.
---
## Examples!
...
$$ \sigma_D= 1.73\sqrt{\frac{1}{500} + \frac{1}{35}} = 0.302$$
$$df = 500 + 35 - 2 = 533$$ 
...
$CI_{\text{Difference}} = (14.09 -13.30) \pm .302(1.96)$
$$[.20, 1.38]$$
...
plot_data %>% 
  ...
  geom_errorbar(aes(ymin=Candy-ci, ymax=Candy+ci), width = .2) + xlab("Age Group") + scale_y_continuous(breaks=seq(5,16,1)) + coord_cartesian( ylim = c(5,16))
```
...
Specifically, this is the assumption that the population is normal -- if the population is normal, then our sampling distribution is **definitely** normal and we can use a *t*-distribution.
