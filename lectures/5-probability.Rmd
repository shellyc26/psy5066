---
title: 'Fisher Tradition and Probability'
output:
...
```{r, echo=FALSE}
set.seed(101719)
X = matrix(NA, ncol = 4, nrow = 10000)
for(S in 1:10000){
  X[S,1] = S
  X[S,2] = rbinom(n = 1, size = S, prob = .5)/S
  X[S,3] = rbinom(n = 1, size = S, prob = 1/6)/S
  X[S,4] = rbinom(n = 1, size = S, prob = .01)/S
}
X = as.data.frame(X); names(X) = c("num", "coin", "die", "scrabble")
```
